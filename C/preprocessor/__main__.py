    ...
    def lex(self, data: Iterator[cindex.Token]) -> Generator[Token, None, None]:
        for x in data:
            if x.kind == TokenKind.PUNCTUATION:
                ...
                if x.spelling == 'pragma':
                    yield Token('PRAGMA', x)
                elif x.spelling == 'cle':
                    yield Token('CLE', x)
                elif x.spelling == 'def':
                    yield Token('DEF', x)
                elif x.spelling == 'begin':
                    yield Token('BEGIN', x)
                elif x.spelling == 'end':
                    yield Token('END', x)
                else:
...
class CLETransformer(Transformer):
    ...
    def clejson(self, items: List[Token]): return json.loads(
        self._deraw(self._hlp(items)))
...
def validate_cle(tree_entry, schema, path: Path, logger: Logger):
    ...
    try:
        ...
        err = SourcedException(
            str(e), [Source(path, Range(Position(tree_entry[1], None), None))])
    if err:
        raise err
...
def source_transform(path: Path, source: str, ttree, astyle: str, schema: Any, logger: Logger) -> Transform:
    if(schema is None):
        ...
        defs = [{"cle-label": x[3], "cle-json": x[4]}
                for x in ttree if x[0] == 'cledef']
    else:
        defs = [{"cle-label": x[3], "cle-json": validate_cle(
            x, schema, path, logger)} for x in ttree if x[0] == 'cledef']
    ...
    while curline < len(unproc):
        ...
        offsetDic[curline+offset] = curline
        curline += 1
    return Transform("\n".join(preproc) + "\n", offsetDic, cast(List[LabelledCleJson], defs))
    ...
    try:
        with open(args.input_file) as f:
            with open(args.schema) as s:
                transform = source_transform(args.input_file, f.read(
                ), ttree, args.annotation_style, json.loads(s.read()), logging.getLogger())
