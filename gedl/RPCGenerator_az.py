...
def RPCGeneratorC(enclave,args,enclaveMap,callerList,calleeList):
    ...
    with open((args.odir + "/" + enclave + "/" + rpccFile),"w") as rpcc_file:
        ...
        rpcc_file.write("typedef struct hash_set {\n")
        rpcc_file.write("\tchar* type;\n")
        rpcc_file.write("\tvoid* data;\n};\n")
        if enclaveMap[enclave][1] != "master":
        ...
        for callerPair in callerList[enclaveIndex]:
            for call in callerPair[2]:
                ...
                rpcc_file.write("#pragma cle begin TAG_REQUEST_" + call[0].upper() + "\n\trequest_" + call[0] + "_datatype req_" + call[0] + ";\n\t#pragma cle end TAG_REQUEST_" + call[0].upper() + "\n")
                rpcc_file.write("\t#pragma cle begin TAG_RESPONSE_" + call[0].upper() + "\n\tresponse_" + call[0] + "_datatype res_" + call[0] + ";\n\t#pragma cle end TAG_RESPONSE_" + call[0].upper() + "\n\n")
                rpcc_file.write("\tstatic struct hash_set* hashmap;\n\t")
                rpcc_file.write("static int size = 1000000;\n\t")
                rpcc_file.write("if(hashmap == NULL) {\n\t")
                rpcc_file.write("hashmap = malloc(size*sizeof(hashmap));\n\t}\n")
                if len(call[2]) == 0:
                    rpcc_file.write("\treq_" + call[0] + ".dummy = 0;\n")
                else:
                    for param in call[2]:
                        rpcc_file.write("\treq_" + call[0] + "." + param[1] + "=" + param[1] + ";\n")
                rpcc_file.write("\tint code = 1;\n\t")
                for param in call[2]:
                    rpcc_file.write("code = (code * req_" + call[0] + "." + param[1] + ") % size;\n\t")
                if(call[5].get('cache', 'yes') != 'no'):
                    rpcc_file.write("if(hashmap[code].data != NULL) {\n\t")
                    rpcc_file.write('\t//printf("Cache Hit");\n\t')
                    rpcc_file.write("\tdouble* pointer = ((double*) hashmap[code].data);\n\t")
                    rpcc_file.write("\treturn *pointer;\n\t}\n")
                ...
                rpcc_file.write("\thashmap[code].data = malloc(sizeof("+ call[1] +"));\n")
                rpcc_file.write("\t*(("+ call[1] +"*) hashmap[code].data) = res_" + call[0] + ".ret;\n")
